I need you to make moduales that can do the following:

render isometric map floor from a given 2d array that specifies what frame in a tile sheet goes where.

Handle a "stationary objects" layer. This layer will be on top of the floor and will contain any objects in the simulation that will stay stationary and be able to be interacted with. The objects should be able to have attributes that can be used to do special interactions. like for example if the stationary object is a conveyour belt, then the player sprite should not be able to enter the same tile that the conveyour belt is in and also the player sprite should be able to see if the conveyour belt has an attribute that says there is a box on it. 

Handle Non-stationary objects. This layer, in terms of depth sorting, would exist on the same level as the other objects layer. This layer would allow you to have objects that can be moved and interacted with. For example a box could be on this layer and a robot could be able to pickup a box, which would entail the box then going ontop of the robot and moving along with the robot when the robot moves. This layer could also have NPC sprites that move by them self. Again, the object or NPC should be able to have attributes. I should be able to set each thing in this layer as collidable or non colidable and if it is collidable then the player sprite should not be able to be in the same square as it at any time. 


Then there should be a player level. This level will have the actuall sprite that the player can move with the blockly interface. There should be an api that allows the blockly interface to send moves and make the sprite move. The sprite should not be able to enter the same square as a collidable object, it cant go off the map (like every object). Any thing about the player sprite will be handled on this level. 

I want this code to be organized and easy to use for the next step of the project so do what ever you must. Weather that be defining the logic for each layer in a different .js file or something else. Just make sure the code has an easy to use and modify api. Remember we are using Phaser.js for the game engine here.

Here is some code from the last version of this project that does the isometric map vuilding and other things. please use this for reference:
